...
def make_env(rank, seed=0, log_dir="./logs/sac"):
    def _init():
...
def main(args):
    ...
    eval_env = Monitor(gym.make("Peak-v4"), filename=os.path.join(args.log_dir, "sac_eval.csv"), info_keywords=('map_complexity',))
    ...
    # Save replay buffer
    model.save_replay_buffer(f"{args.save_path}_replay_buffer.pkl")
    print(f"Training complete. Model saved to {args.save_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train SAC on PeakEnv with resume support")
    parser.add_argument("--resume", type=str, default=None, help="Path to checkpoint zip to resume from")
    parser.add_argument("--resume_buffer", type=str, default=None, help="Path to replay buffer pickle to load")
    parser.add_argument("--seed", type=int, default=0, help="Random seed")
    parser.add_argument("--total_timesteps", type=int, default=200_000, help="Total timesteps for learning")
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/sac", help="Directory to save checkpoints")
    ...
    parser.add_argument("--log_dir", type=str, default="logs/sac", help="Directory for Monitor logs")
    parser.add_argument("--tensorboard_log", type=str, default="tensorboard/sac", help="Tensorboard log dir")
    parser.add_argument("--save_path", type=str, default="sac_peak_final", help="Path to save final model")
